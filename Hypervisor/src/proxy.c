...
extern u64 autoRestoreNoAgainCnt, autoRestoreAgainCnt, autoRestoreSingleStepCnt,EL2SingeStepCnt;
    ...
    switch (request->opcode) {
        ...
        case P_HV_TRANSLATE:
            reply->retval = hv_translate(request->args[0], request->args[1], request->args[2],
                                         (void *)request->args[3]);
            break;
        case P_HV_TRANSLATE_EL0:
            reply->retval = analyzeEL0Addr(request->args[0]);
            break;
        case P_HV_PT_WALK:
        ...
        case P_BRKPOINT_INVOKE:
            reply->retval = handleBreakpointInvocation(request->args[0]);
            break;
        case P_BRKPOINT:
            // reply->retval = brkpoint(request->args[0]);
            reply->retval = setBreakpointByAddr(request->args[0]);
            break;
        case P_BRKPOINT_IDA:
            reply->retval = setBreakpointByAddr_IDA(request->args[0]);
            break;
        case P_BRKPOINT_IDA_BRCODE:
            reply->retval = set_breakpoint_in_ida_with_brcode(request->args[0], request->args[1]);
            break;
        case P_LS_BRKPOINT:
            reply->retval = listBreakpoints();
            break;  
        case P_SET_BRKPOINT_STATUS:
            reply->retval = setBreakpointStatus(request->args[0], request->args[1]);
            break;
        case P_PRINT_KASLR_OFFSET:
            reply->retval = (int) kaslrOffset;
            break;
        case P_SINGLE_STEP:
            reply->retval = handleSingleStepReinstrument();
            break;
        case P_PASS_BACKUPTEXT_BASE:
            reply->retval = saveBackuptextBase(request->args[0]);
            break;
        case P_SET_KASLR_OFFSET:
            reply->retval = setKaslrOffset(request->args[0]);
            break;
        case P_PRINT_AUTORESTORE_CNT:
            printf("\nAutoRestoreCnt and NO AGAIN: 0x%lx\n", autoRestoreNoAgainCnt);
            printf("[Double Cross Rewrite] AutoRestoreCnt and AGAIN: 0x%lx\n", autoRestoreAgainCnt);
            printf("[Single Step] autoRestoreSingleStepCnt: 0x%lx, EL2SingleStepHandlingCnt: 0x%lx\n", autoRestoreSingleStepCnt,EL2SingeStepCnt);
            // reply->retval = 0LL; 
            break;
        case P_GET_PA:
            getPA(request->args[0]);
            break;
        case P_SWITCH_EXCEPTION_ROUTE:
            reply->retval = switchExceptionRoute();
            break;
        case P_SET_EL2Handler_For_EL1Handler_BASE:
            reply->retval = setEL2handlerForEL1handlerBase(request->args[0]);
            break;
        case P_ANALYZE_VADDR:
            reply->retval = analyzeVAddr(request->args[0]);
            break;
        case P_MAP_EL1Handler_PTE:
            reply->retval = mapEL1HandlerPTE(request->args[0], request->args[1]);
            break;
        case P_IPA_WALK:
            // reply->retval = ipawalk(request->args[0]);
            // reply->retval = hv_l3(request->args[0]); 
            break;
        case P_HCR_SET_API_APK:
            reply->retval = setHCR_APIAPK(request->args[0], request->args[1]);
            // reply->retval = setHCR_APIAPK_ForAllCPUs(request->args[0], request->args[1]);
            break;
        case P_SET_Running_Kernel_EL2Base:
            reply->retval = setRunningKernelEl2Base(request->args[0]);
            printf("[hym1] RunningKernelEl2Base set to 0x%lx\n", reply->retval);
            break;
        case P_PRINT_EXTERNAL_ANALYSIS:
            reply->retval = 0;
            printBinOrderKextNames();
            break;
        case P_PRINT_MEM:
            reply->retval = 0;
            hexdump_1(request->args[0], request->args[1], request->args[2]);
            break;
        case P_CLEAR_DATA_CACHE:
            reply->retval = 0;
            clearDataCache(request->args[0]);
            break;
        case P_SET_DEBUG_PID:
            reply->retval = (u64) setDebugPid(request->args[0]);
            break;
        case P_CPUFREQ_INIT:
            reply->retval = cpufreq_init();
            break;
        case P_GL_WRITE32:
            reply->retval = gl2_call(glwrite32, request->args[0], request->args[1], 0, 0);
            break;
        case P_MY_HV_MAP:
            reply->retval = myhv_map(request->args[0], request->args[1], request->args[2], request->args[3]);
            break;
        case P_SET_SHADOW_PAGETABLE:
            set_backup_pagetable_for_cpuid(request->args[0]);
            break;
        case P_INIT_SHADOW_PAGETABLE:
            reply->retval = init_backup_pagetable();
            break;
        case P_VTT_WALK:
            reply->retval = myhv_pt_walk(request->args[0]);
            break;
        case P_GET_PID:
            reply->retval = getCurrentPID();
            break;
        case P_SET_UVA_MEM_HOOK:
            reply->retval = uva_hook(request->args[0], request->args[1], request->args[2]); // uva, size
            break;
        case P_SET_UVA_MEM_UNHOOK:
            reply->retval = uva_unhook(request->args[0], request->args[1], request->args[2]);
            break;
        case P_SET_IPA_MEM_UNHOOK:
            reply->retval = ipa_unhook(request->args[0], request->args[1]); // va, ipa
            break;
        case P_UVA_WALK:
            reply->retval = uva_walk(request->args[0], request->args[1]);
            break;
        case P_MY_PT_WALK:
            reply->retval = pt_walk(request->args[0], request->args[1], request->args[2], request->args[3], request->args[4]);
            break;
        case P_JUDGE_IF_NEXT_SYSCALL:
            reply->retval = judge_if_not_in_the_same_syscall(request->args[0]);
            break;
        case P_SET_IPA_MEM_HOOK:
            reply->retval = ipa_hook(request->args[0], request->args[1]); // va, ipa
            break;
        default:
