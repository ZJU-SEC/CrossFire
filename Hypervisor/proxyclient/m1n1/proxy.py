...
class M1N1Proxy(Reloadable):
    ...
    P_BRKPOINT_IDA_BRCODE = 0x22ff
    P_BRKPOINT = 0x2300
    P_PRINT_KASLR_OFFSET = 0x2301
    P_PASS_BACKUPTEXT_BASE = 0x2302
    P_SET_KASLR_OFFSET = 0x2303
    P_PRINT_AUTORESTORE_CNT = 0x2304
    P_GET_PA = 0x2305
    P_SWITCH_EXCEPTION_ROUTE = 0x2306
    P_SET_EL2Handler_For_EL1Handler_BASE = 0x2307
    P_ANALYZE_VADDR = 0x2308
    P_MAP_EL1Handler_PTE = 0x2309
    P_IPA_WALK = 0x230a
    P_HCR_SET_API_APK = 0x230b
    P_SET_Running_Kernel_EL2Base = 0x230c
    P_PRINT_EXTERNAL_ANALYSIS = 0x230d
    P_BRKPOINT_IDA = 0x230e
    P_LS_BRKPOINT = 0x230f
    P_SET_BRKPOINT_STATUS = 0x2310
    P_BRKPOINT_INVOKE = 0x2311
    P_SINGLE_STEP = 0x2312
    P_PRINT_MEM = 0x2313
    P_CLEAR_DATA_CACHE = 0x2314
    P_SET_DEBUG_PID = 0x2315

    P_GL_WRITE32 = 0x2318
    P_HV_TRANSLATE_EL0 = 0x2319
    P_MY_HV_MAP = 0x231a
    P_SET_SHADOW_PAGETABLE = 0x231b
    P_INIT_SHADOW_PAGETABLE = 0x231c
    P_VTT_WALK = 0x231d
    P_GET_PID = 0x231e
    P_SET_UVA_MEM_HOOK = 0x231f
    P_SET_UVA_MEM_UNHOOK = 0x2320
    # P_SET_SS_MODE = 0x2321 
    P_UVA_WALK = 0x2322
    P_MY_PT_WALK = 0x2323
    P_SET_IPA_MEM_UNHOOK = 0x2324
    P_JUDGE_IF_NEXT_SYSCALL = 0x2325
    P_SET_IPA_MEM_HOOK = 0x2326
 ...
 and return the result'''
    ...
    def print_cov(self):
        return self.request(self.P_PRINT_COV)
    ...
    def myhv_map(self, from_, to, size, incr):
        return self.request(self.P_MY_HV_MAP, from_, to, size, incr)
 ...
 stage 1 only if s1, for write if w'''
    ...
    def hv_transel0(self, addr):
        return self.request(self.P_HV_TRANSLATE_EL0, addr)
    ...
    def dump(self, addr, size):
        start = addr
        step = 1*1024*1024
        with open("memdump "+hex(addr)+" "+hex(addr + size), "wb") as f:
            i = 0
            while i<size:
                # if i % (1024*1024*256) == 0:
                print("[*] addr 0x%x ~ 0x%x, %d KB, %d M, %d G"%(addr+i, addr+i+step, i//1024, i//1024//1024, i//1024//1024//1024))
                try:
                    data = self.iface.readmem(addr+i, step)
                    f.write(data)
                except Exception as e:
                    print(e)
                    break
                i += step
        print("0x%x ~ 0x%x"%(start, addr+i))
        f.close()
        return True
    def remove_null_bytes(self, data):
        # 
        null_byte_index = data.find(b'\x00')

        # 
        if null_byte_index != -1:
            result = data[:null_byte_index]
        else:
            # 
            result = data
        return result
    def breakpoint(self, addr):
        pass
    def bi(self, addr):
        retval = self.request(self.P_BRKPOINT_IDA, addr)
        print("[+] breakpoint in ida:",hex(addr), "real addr:",hex(retval))
        return retval
    def bic(self, addr, brcode):
        if brcode < 0x100:
            print("[-] brcode too small, use >= 0x100")
            return -1
        retval = self.request(self.P_BRKPOINT_IDA_BRCODE, addr, brcode)
    def br(self, addr):
        retval = self.request(self.P_BRKPOINT, addr)
        print("[+] breakpoint:",hex(addr))
        return retval
    def step(self):
        retval = self.request(self.P_SINGLE_STEP)
        return retval
    def lb(self):
        retval = self.request(self.P_LS_BRKPOINT)
        return retval
    def pm(self, va, size=0x100):
        max_size = 0x4000;
        retval = self.request(self.P_PRINT_MEM, va, size, max_size)
        return retval
    def cdc(self, addr):#clear data cache
        retval = self.request(self.P_CLEAR_DATA_CACHE, addr)
        return retval
    def sbs(self, idx, status):
        retval = self.request(self.P_SET_BRKPOINT_STATUS, idx, status)
        return retval
    def dpid(self, pid):# set debug pid
        retval = self.request(self.P_SET_DEBUG_PID, pid)
        return retval
    def bInvoke(self, elr):
        retval = self.request(self.P_BRKPOINT_INVOKE, elr)
        return retval
    def setKaslr(self, kaslrOffset):
        retval = self.request(self.P_SET_KASLR_OFFSET, kaslrOffset)
        print("[+] kaslr offset set to be: ", hex(retval))
        return retval
    def pkaslr(self):
        retval = self.request(self.P_PRINT_KASLR_OFFSET)
        print("[+] kaslr offset: ", hex(retval))
        return retval
    def pcnt(self):
        retval = self.request(self.P_PRINT_AUTORESTORE_CNT) # printf in C not here
        # print("[+] auto restore cnt: ", hex(retval))
        return retval
    def pass_backuptext_base(self, base):
        return self.request(self.P_PASS_BACKUPTEXT_BASE, base)
    def pa(self, addr):
        return self.request(self.P_GET_PA, addr)
    def eroute(self):
        # self.hmap()
        return self.request(self.P_SWITCH_EXCEPTION_ROUTE)
    def vaddr(self, addr):
        return self.request(self.P_ANALYZE_VADDR, addr)
    def setEL2handlerForEL1handlerBase(self, base):
        print("[*] el1handlerBase set to be {}".format(hex(base)));
        return self.request(self.P_SET_EL2Handler_For_EL1Handler_BASE, base)
    def hmap(self):
        print("[*] init EL1handler PTE, PA = {}, VA = {}, assume handler size <= 16*1000 kb".format(hex(self.EL1HandlerEL2base), hex(self.EL1HandlerVAddrBase)))
        return self.request(self.P_MAP_EL1Handler_PTE, self.EL1HandlerEL2base, self.EL1HandlerVAddrBase)
    def ipawalk(self, ipa):
        return self.request(self.P_IPA_WALK, ipa)
    def hcrik(self, api, apk):
        return self.request(self.P_HCR_SET_API_APK, api, apk)
    def setRunningKernelEL2Base(self, addr):
        return self.request(self.P_SET_Running_Kernel_EL2Base, addr)
    def peia(self):# PRINT_EXTERNAL_ANALYSIS
        return self.request(self.P_PRINT_EXTERNAL_ANALYSIS)
    def glw32(self, addr, value):
        return self.request(self.P_GL_WRITE32)
    def init_spt(self):
        return self.request(self.P_INIT_SHADOW_PAGETABLE)
    def set_spt(self, cpuid):
        return self.request(self.P_SET_SHADOW_PAGETABLE, cpuid)
    def vttwalk(self, addr):
        return self.request(self.P_VTT_WALK, addr)
    def get_pid(self):
        return self.request(self.P_GET_PID)
    def uvahook(self, addr, size, ttbr0):
        # 
        if size > addr:
            size = size - addr
        return self.request(self.P_SET_UVA_MEM_HOOK, addr, size, ttbr0)
    
    def uvaunhook(self, addr, size, ttbr0):
        if size > addr:
            size = size - addr
        return self.request(self.P_SET_UVA_MEM_UNHOOK, addr, size, ttbr0)
    def ipaunhook(self, va, ipa):
        return self.request(self.P_SET_IPA_MEM_UNHOOK, va, ipa)
    def ipahook(self,va,ipa):
        return self.request(self.P_SET_IPA_MEM_HOOK, va, ipa)
    
    # def mode(self, mode):
    #     return self.request(self.P_SET_SS_MODE, mode)

    def uva_walk(self, addr, ttbr0):
        return self.request(self.P_UVA_WALK, addr, ttbr0)

    def pt_walk(self, addr, ttbr, level,  start_level, return_pte_address):
        return self.request(self.P_MY_PT_WALK, addr, ttbr, level, start_level, return_pte_address)
    
    def judge_if_next_syscall(self, cpu_id):
        return self.request(self.P_JUDGE_IF_NEXT_SYSCALL, cpu_id)
