...
class ProxyUtils(Reloadable):
    ...
    # def hvc_continue(self, ctx):
    #     ctx.elr = ctx.elr+4
    def print_context(self, ctx, is_fault=True, addr=lambda a: f"0x{a:x}", sym=None, num_ctx=9, kaslr = -1):
        ...
        print(f"  context_addr = 0x{ctx.sp[2]:x}")
        ...
        for i in range(0, 31, 4):
            j = min(30, i + 3)
            print(f"  {f'x{i}-x{j}':>7} = {' '.join(f'0x{r:016x}' for r in ctx.regs[i:j + 1])}")
        ...
        if ctx.elr_phys:
            ...
            self.disassemble_at(ctx.elr_phys + 8 * off, num_ctx * 8, ctx.elr, ctx.elr + 8 * off, sym=sym, kaslr=kaslr)
